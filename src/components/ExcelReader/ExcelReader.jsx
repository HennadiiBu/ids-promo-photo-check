...
export default function ExcelCardsGrouped({ openFullScreenMode }) {
  ...
  const exportUpdatedExcel = () => {
    ...
    const updated = data
      ...
      .filter(
        row => row[NEW_COL_EXISTING] === 5 || row[NEW_COL_EXISTING] === 1
      ); // фильтруем строки

    if (!updated.length)
      return alert('Нет строк с данными 5 или 1 для экспорта.');
  ...
  // ---- Группировка по Код ТТ, только пустые NEW_COL_EXISTING ----
  const groupedDataForDisplay = Object.values(
    data
      .filter(row => !row[NEW_COL_EXISTING]) // только пустые
      .reduce((acc, row) => {
        const code = row[CODE_COL] || 'no_code';
        if (!acc[code]) acc[code] = { code, rows: [] };
        acc[code].rows.push(row);
    ...
    <div style={{ padding: 16, fontFamily: 'sans-serif' }}>
      ...
      {!data.length ? (
        ...
        <>
          ...
          >
            {groupedDataForDisplay.map((group, idx) => {
                ...
                >
                  ...
                  <div style={{ position: 'absolute', bottom: 12, left: 12 }}>
                    ...
                    >
                      Умови УПЦ виконані?
                      {answers[code] &&
                        !['5', '1'].includes(String(answers[code])) && (
